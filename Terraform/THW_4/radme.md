...
На данном этапе сначала ловил ошибку с попыткой установить пакеты из официального репозитория Hashicorp в попытках разобраться в вопросе зачистил весь кэш в terraform, не выполнив предварительно terraform destroy. В итоге разобрался в том что в модуле не указан провайдер и вся проблема там. В последствии поймал ошибку с ограничением количества сетей в yandex cloud, после ручной зачистки ресурсов на портале yandex cloud получил необходимый результат.
...
1. **Ошибка с провайдером HashiCorp:**
   Сначала `terraform init` намертво зависал и выдавал ошибку, пытаясь скачать плагин Яндекса с официального заблокированного сайта HashiCorp. Пока пытался разобраться, случайно удалил всю папку кэша `.terraform` и файлы состояния `terraform.tfstate`, забыв перед этим сделать очистку через `terraform destroy`. Из-за этого Terraform «забыл» про уже созданные в облаке серверы, и они остались там висеть «сиротами».
