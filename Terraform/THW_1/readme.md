...
**Ответ:** Личную и секретную информацию допустимо сохранять в файлах с расширением `*.tfvars` (например, `terraform.tfvars`) или `*.auto.tfvars` (например, `secrets.auto.tfvars`). В предоставленном файле `.gitignore` явно прописаны эти маски, что гарантирует игнорирование данных файлов утилитой Git. Они никогда не попадут в публичный репозиторий на GitHub, но локально Terraform их успешно прочитает. В данном случае в файле .gitignore секретные данные предлагается хранить в файле personal.auto.tfvars.
