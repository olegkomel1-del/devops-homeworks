...
### Шаг 1: Написал динамические выражения для генерации последовательностей в terraform console.

```bash
terraform console
```

### Шаг 2: Сгенерировал последовательный список от "rc01" до "rc99" с автоматическим добавлением ведущего нуля для чисел первого десятка.
...
### Шаг 3: Сгенерировал список от "rc01" до "rc96" с применением сложной фильтрации. С помощью оператора остатка от деления `% 10` вычислил последнюю цифру каждого числа, исключил запрещенные окончания (`0`, `7`, `8`, `9`) через функцию `!contains()` и добавил жесткое условие-исключение для сохранения элемента `rc19` через логическое ИЛИ (`||`).
